...
def CalcMap_cuda_topk(qB, rB, queryL, retrievalL, topk=1000, valid_bit_mask=None):
    # qB: {-1,+1}^{mxq}
    # rB: {-1,+1}^{nxq}
    # queryL: {0,1}^{mxl}
    # retrievalL: {0,1}^{nxl}
    ...
    for iter in range(num_query):
        ...
        if valid_bit_mask is None:
            ...
            hamm = CalcHammingDist_cuda(qB[None, iter, :], rB, valid_bit_mask=valid_bit_mask[None, iter, :])
        topk_id = torch.argsort(hamm)[:topk]
        gnd = gnd[topk_id]
        hamm = hamm[topk_id]
        tsum = torch.sum(gnd).cpu().data.numpy()
        if tsum == 0:
...
def CalcMap_cuda_topk_rerank(qB, rB, qC, rC, queryL, retrievalL, topk=1000, valid_bit_mask=None):
    # qB: {-1,+1}^{mxq}, B for binary code
    # rB: {-1,+1}^{nxq}
    # qC: {-1,+1}^{mxq}, C for continuous code
    # rC: {-1,+1}^{nxq}
    ...
    for iter in range(num_query):
        ...
        if tsum == 0:
            continue
        if valid_bit_mask is None:
            l2m = CalcL2Dist_cuda(qC[None, iter, :], rC[topk_id, :])
        else: 
            l2m = CalcL2Dist_cuda(qC[None, iter, :], rC[topk_id, :], valid_bit_mask=valid_bit_mask[None, iter, :])
        ind = torch.argsort(l2m)
        gnd = gnd[ind]
        gnd = gnd.cpu().data.numpy()
        # print(tsum)
